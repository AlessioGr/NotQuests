...
dependencies {
    ...
    // Adventure: pinned to the Adventure API used by this branch. Do NOT move to
    // 5.x — Paper provides 4.x at runtime, so a 5.x compile target would break against the server.
    implementation("net.kyori:adventure-api:4.26.1") {}

    // InvUI 1.49 + the 1.21.11 inventory-access adapter. InvUI 2.x references Paper
    // DataComponentTypes, which is not available across all supported 1.21.x backport targets.
