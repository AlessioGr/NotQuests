...
public class DateCondition extends Condition {
    ...
    public static void handleCommands(NotQuests main, PaperCommandManager<CommandSender> manager, Command.Builder<CommandSender> builder, ConditionFor conditionFor) {
        ...
        final CommandFlag<Integer> month = CommandFlag
                ...
                .build();
        final CommandFlag<Integer> day = CommandFlag
                .newBuilder("day")
                ...
                .build();
        final CommandFlag<Integer> hours = CommandFlag
                .newBuilder("hours")
        ...
        final CommandFlag<Integer> minutes = CommandFlag
                .newBuilder("minutes")
                .withArgument(IntegerArgument.<CommandSender>newBuilder("minutes").withMin(0).withMax(60))
                .withDescription(ArgumentDescription.of("Enter minutes."))
                .build();

        final CommandFlag<Integer> seconds = CommandFlag
                .newBuilder("seconds")
                .withArgument(IntegerArgument.<CommandSender>newBuilder("seconds").withMin(0).withMax(60))
        ...
        manager.command(builder
                .argument(StringArgument.<CommandSender>newBuilder("Date operation").withSuggestionsProvider(
                        (context, lastString) -> {
                            final List<String> allArgs = context.getRawInput();
                            main.getUtilManager().sendFancyCommandCompletion(context.getSender(), allArgs.toArray(new String[0]), "[Date operation]", "<optional flags>");
                ...
                ).single().build(), ArgumentDescription.of("Date operation"))
                .flag(year)
                .flag(month)
                .flag(day)
                .flag(hours)
    ...
    public void load(FileConfiguration configuration, String initialPath) {
        setYear(
                configuration.getInt(initialPath + ".specifics.year", -1)
        );
        setMonth(
                configuration.getInt(initialPath + ".specifics.month", -1)
        );
        setDay(
                configuration.getInt(initialPath + ".specifics.day")
        );
        setHours(
                configuration.getInt(initialPath + ".specifics.hours")
        );
        setMinutes(
                configuration.getInt(initialPath + ".specifics.minutes")
        );
        setSeconds(
                configuration.getInt(initialPath + ".specifics.seconds")
        );
        setOperation(
                configuration.getString(initialPath + ".specifics.operation", "")
        );
