...
public class GUIManager {
    ...
    public void constructInterfaces(){
        ...
        mainInterface = constructMainInterface(main.getLanguageManager().getComponent("gui.main.title", null ));
        ...
        previewQuestInterface = ChestInterface.builder()
                ...
                .addTransform(PaperTransform.chestFill(
                        ItemStackElement.of(separatorItemStack3)
                ))
                .addTransform((pane, view) -> {
                    ...
                    if (main.getConfiguration().isGuiQuestPreviewDescription_enabled()) {
                        ...
                        itemMeta.displayName(main.getLanguageManager().getComponent("gui.previewQuest.button.description.name", player, questPlayer, quest));

                        if(!quest.getQuestDescription().isBlank()){
                            List<String> loreStringList = main.getLanguageManager().getStringList("gui.previewQuest.button.description.lore", player, questPlayer, quest);
                            List<Component> lore = new ArrayList<>();

                            for(String loreString : loreStringList){
                                if(loreString.contains("%WRAPPEDQUESTDESCRIPTION%")){
                                    for(String questDescriptionLine : quest.getQuestDescriptionList(main.getConfiguration().guiQuestDescriptionMaxLineLength)){
                                        lore.add(main.parse(loreString.replace("%WRAPPEDQUESTDESCRIPTION%", "") + questDescriptionLine).decoration(TextDecoration.ITALIC, false));
                                    }
                                }else{
                            ...
                            itemMeta.lore(main.getLanguageManager().getComponentList("gui.previewQuest.button.description.lore-if-empty", player, questPlayer, quest));
                        }

                        itemStack.setItemMeta(itemMeta);


                        result = result.element(ItemStackElement.of(itemStack
                        ), main.getLanguageManager().getInt("gui.previewQuest.button.description.x"), main.getLanguageManager().getInt("gui.previewQuest.button.description.y"));
                    }

                    if (main.getConfiguration().isGuiQuestPreviewRewards_enabled()) {
                        ItemStack itemStack = new ItemStack(Material.EMERALD);
                        ItemMeta itemMeta = itemStack.getItemMeta();
                        itemMeta.displayName(main.getLanguageManager().getComponent("gui.previewQuest.button.rewards.name", player, questPlayer, quest));

                        if(!quest.getRewards().isEmpty()){
                            List<String> loreStringList = main.getLanguageManager().getStringList("gui.previewQuest.button.rewards.lore", player, questPlayer, quest);
                            List<Component> lore = new ArrayList<>();
                            ...
                            itemMeta.lore(main.getLanguageManager().getComponentList("gui.previewQuest.button.rewards.lore-if-empty", player, questPlayer, quest));
                        }

                        itemStack.setItemMeta(itemMeta);

                        result = result.element(ItemStackElement.of(itemStack
                        ), main.getLanguageManager().getInt("gui.previewQuest.button.rewards.x"), main.getLanguageManager().getInt("gui.previewQuest.button.rewards.y"));
                    }

                    if (main.getConfiguration().isGuiQuestPreviewRequirements_enabled()) {
                        ItemStack itemStack = new ItemStack(Material.IRON_BARS);
                        ItemMeta itemMeta = itemStack.getItemMeta();
                        itemMeta.displayName(main.getLanguageManager().getComponent("gui.previewQuest.button.requirements.name", player, questPlayer, quest));

                        if(!quest.getRequirements().isEmpty()){
                            List<String> loreStringList = main.getLanguageManager().getStringList("gui.previewQuest.button.requirements.lore", player, questPlayer, quest);
                            List<Component> lore = new ArrayList<>();
                            ...
                            itemMeta.lore(main.getLanguageManager().getComponentList("gui.previewQuest.button.requirements.lore-if-empty", player, questPlayer, quest));
                        }

                        itemStack.setItemMeta(itemMeta);

                        result = result.element(ItemStackElement.of(itemStack
                        ), main.getLanguageManager().getInt("gui.previewQuest.button.requirements.x"), main.getLanguageManager().getInt("gui.previewQuest.button.requirements.y"));
                    ...
                    confirmTakeItemMeta.displayName(main.getLanguageManager().getComponent("gui.previewQuest.button.confirmTake.name", player, questPlayer, quest));
                    confirmTakeItemMeta.lore(main.getLanguageManager().getComponentList("gui.previewQuest.button.confirmTake.lore", player, questPlayer, quest));
                    confirmTakeItemStack.setItemMeta(confirmTakeItemMeta);

                    result = result.element(ItemStackElement.of(confirmTakeItemStack,
                            (clickHandler) -> {
                                player.chat("/notquests take " + quest.getQuestName());
                                clickHandler.viewer().close();
                            }
                    ), main.getLanguageManager().getInt("gui.previewQuest.button.confirmTake.x"), main.getLanguageManager().getInt("gui.previewQuest.button.confirmTake.y"));
                    ...
                    cancelTakeItemMeta.displayName(main.getLanguageManager().getComponent("gui.previewQuest.button.cancelTake.name", player, questPlayer, quest));
                    cancelTakeItemMeta.lore(main.getLanguageManager().getComponentList("gui.previewQuest.button.cancelTake.lore", player, questPlayer, quest));
                    cancelTakeItemStack.setItemMeta(cancelTakeItemMeta);

                    result = result.element(ItemStackElement.of(cancelTakeItemStack,
                            (clickHandler) -> {
                                clickHandler.viewer().close();
                            }
                    ), main.getLanguageManager().getInt("gui.previewQuest.button.cancelTake.x"), main.getLanguageManager().getInt("gui.previewQuest.button.cancelTake.y"));
        ...
        abortQuestInterface = ChestInterface.builder()
                // This interface will have one row.
                .rows(3)
                // This interface will update every five ticks.
                //.updates(true, 5)
                ...
                .addTransform((pane, view) -> {
                    ...
                    final ActiveQuest activeQuest = view.arguments().get(ArgumentKey.of("activeQuest", ActiveQuest.class));
                    ...
                    confirmAbortItemMeta.displayName(main.getLanguageManager().getComponent("gui.abortQuest.button.confirmAbort.name", player, questPlayer, activeQuest));
                    confirmAbortItemMeta.lore(main.getLanguageManager().getComponentList("gui.abortQuest.button.confirmAbort.lore", player, questPlayer, activeQuest));
                    confirmAbortItemStack.setItemMeta(confirmAbortItemMeta);

                    result = result.element(ItemStackElement.of(confirmAbortItemStack,
                            (clickHandler) -> {
                                questPlayer.failQuest(activeQuest);
                                main.sendMessage(player, main.getLanguageManager().getString("chat.quest-aborted", player, activeQuest));
                                clickHandler.viewer().close();
                            }
                    ), main.getLanguageManager().getInt("gui.abortQuest.button.confirmAbort.x"), main.getLanguageManager().getInt("gui.abortQuest.button.confirmAbort.y"));
                    ...
                    cancelAbortItemMeta.displayName(main.getLanguageManager().getComponent("gui.abortQuest.button.cancelAbort.name", player, questPlayer, activeQuest));
                    cancelAbortItemMeta.lore(main.getLanguageManager().getComponentList("gui.abortQuest.button.cancelAbort.lore", player, questPlayer, activeQuest));
                    cancelAbortItemStack.setItemMeta(cancelAbortItemMeta);

                    result = result.element(ItemStackElement.of(cancelAbortItemStack,
                            (clickHandler) -> {
                                clickHandler.viewer().close();
                            }
                    ), main.getLanguageManager().getInt("gui.abortQuest.button.cancelAbort.x"), main.getLanguageManager().getInt("gui.abortQuest.button.cancelAbort.y"));
                ...
                .addTransform((pane, view) -> {
                    ...
                    for(final ActiveObjective activeObjective : activeQuest.getActiveObjectives()){
                        ...
                        if (activeObjective.isUnlocked()) {
                            ...
                            if(!activeObjective.getObjective().getDescription().isBlank()){
                                ...
                                itemMeta.lore(main.getLanguageManager().getComponentList("gui.progress.button.unlockedObjective.lore-if-description-empty", player, questPlayer, activeQuest, activeObjective));
                            }
                            ...
                            itemMeta.displayName(main.getLanguageManager().getComponent("gui.progress.button.lockedObjective.name", player, questPlayer, activeQuest, activeObjective));
                            itemMeta.lore(main.getLanguageManager().getComponentList("gui.progress.button.lockedObjective.lore", player, questPlayer, activeQuest, activeObjective));
                            itemStack.setItemMeta(itemMeta);
                    ...
                    for (final ActiveObjective activeObjective : activeQuest.getCompletedObjectives()) {
                        ...
                        if(!activeObjective.getObjective().getDescription().isBlank()){
                            ...
                            itemMeta.lore(main.getLanguageManager().getComponentList("gui.progress.button.completedObjective.lore-if-description-empty", player, questPlayer, activeQuest, activeObjective));
                        }
                        ...
                        result = result.element(ItemStackElement.of(itemStack
                        ), counterX++, counterY); //TODO: Positions
        ...
        selectiveTakeQuestsInterface = ChestInterface.builder()
                ...
                .addTransform((pane, view) -> {
                    ...
                    final Player player = view.arguments().get(ArgumentKey.of("player", Player.class));
                    final QuestPlayer questPlayer = view.arguments().get(ArgumentKey.of("questPlayer", QuestPlayer.class));
                    final ArrayList<Quest> selectedQuests = view.arguments().get(ArgumentKey.of("quests", ArrayList.class));
                    ...
                    ArrayList<ItemStackElement<ChestPane>> list = new ArrayList<>() {
                        {
                            ...
                            for (final Quest quest : main.getQuestManager().getQuestsFromListWithVisibilityEvaluations(questPlayer, selectedQuests)) {
                                ...
                                List<String> loreStringList = main.getLanguageManager().getStringList("gui.availableQuests.button.questPreview.lore", player, quest);
                                List<Component> lore = new ArrayList<>();
                                ...
                                if (questPlayer != null && questPlayer.hasAcceptedQuest(quest)) {
                                    itemMeta.displayName(main.getLanguageManager().getComponent("gui.availableQuests.button.questPreview.name-if-accepted", player, quest));
                                }else{
                                    itemMeta.displayName(main.getLanguageManager().getComponent("gui.availableQuests.button.questPreview.name-if-not-accepted", player, quest));
                                }
                    ...
                    PaginatedTransform<ItemStackElement<ChestPane>, ChestPane, PlayerViewer> paginatedTransform = new PaginatedTransform<>(
                            Vector2.at(1, 1), Vector2.at(7, 4), list);
    ...
    public ChestInterface constructMainInterface(Component title){
        ...
        return ChestInterface.builder()
                ...
                .addTransform((pane, view) -> {
                    ChestPane result = pane;
                    for(int x=0; x<=8; x++){
                        result = result.element(ItemStackElement.of(separatorItemStack1
                        ),x, 0);
                    }
                    for(int y=0; y<=5; y++){
                        result = result.element(ItemStackElement.of(separatorItemStack1
                    ...
                    for(int y=0; y<=5; y++){
                        result = result.element(ItemStackElement.of(separatorItemStack2
                        ), 1, y);
                    }
                    for(int y=0; y<=5; y++){
                        result = result.element(ItemStackElement.of(separatorItemStack1
                ...
                .addTransform((pane, view) -> {
                    ...
                    final String paneType = view.arguments().getOrDefault(ArgumentKey.of("paneType", String.class), "");


                    ItemMeta coinsItemMeta = coins.getItemMeta();
                    coinsItemMeta.displayName(main.getLanguageManager().getComponent("gui.main.button.questpoints.name", player, questPlayer));
                    coinsItemMeta.lore(main.getLanguageManager().getComponentList("gui.main.button.questpoints.lore", player, questPlayer));
                    coins.setItemMeta(coinsItemMeta);

                    result = pane.element(ItemStackElement.of(coins
                    ), main.getLanguageManager().getInt("gui.main.button.questpoints.x"), main.getLanguageManager().getInt("gui.main.button.questpoints.y"));
                    ...
                    takeItemMeta.displayName(main.getLanguageManager().getComponent("gui.main.button.takequest.name", player, questPlayer));
                    takeItemMeta.lore(main.getLanguageManager().getComponentList("gui.main.button.takequest.lore", player, questPlayer));
                    ...
                    abortItemMeta.displayName(main.getLanguageManager().getComponent("gui.main.button.abortquest.name", player, questPlayer));
                    abortItemMeta.lore(main.getLanguageManager().getComponentList("gui.main.button.abortquest.lore", player, questPlayer));
                    ...
                    activeQuestsItemMeta.displayName(main.getLanguageManager().getComponent("gui.main.button.activequests.name", player, questPlayer));
                    activeQuestsItemMeta.lore(main.getLanguageManager().getComponentList("gui.main.button.activequests.lore", player, questPlayer));
                ...
                .addTransform((pane, view) -> { //For the actual content
                    final String paneType = view.arguments().getOrDefault(ArgumentKey.of("paneType", String.class), "");
                    if(paneType.equalsIgnoreCase("takequest")){
                        if(view.arguments().contains(ArgumentKey.of("category", Category.class))){
                            final Category category = view.arguments().get(ArgumentKey.of("category", Category.class));
    ...
    public ChestPane getActiveQuestsPane(ChestPane pane, InterfaceView<ChestPane, PlayerViewer> view){
        final Player player = view.arguments().get(ArgumentKey.of("player", Player.class));
        ...
        if(view.arguments().contains(ArgumentKey.of("questPlayer", QuestPlayer.class))){
            ...
            PaginatedTransform<ItemStackElement<ChestPane>, ChestPane, PlayerViewer> paginatedTransform = new PaginatedTransform<>(
                    Vector2.at(3, 1), Vector2.at(8, 5), list);
            return paginatedTransform.apply(pane, view);
        }
        return pane;
    }
            ...
            ArrayList<ItemStackElement<ChestPane>> list = new ArrayList<>() {
                {
                    for (final Category category : main.getDataManager().getCategories()) {
                        ...
                        add(ItemStackElement.of(itemStack,
                                (clickHandler) -> {
                                    final HashMapInterfaceArguments arguments = HashMapInterfaceArguments
                                            .with(ArgumentKey.of("player", Player.class), clickHandler.view().arguments().get(ArgumentKey.of("player", Player.class)))
                                            .with(ArgumentKey.of("questPlayer", QuestPlayer.class), clickHandler.view().arguments().get(ArgumentKey.of("questPlayer", QuestPlayer.class)))
                                            .with(ArgumentKey.of("paneType", String.class), "takequest")
                                            .with(ArgumentKey.of("category", Category.class), category).build();
                                    constructMainInterface(main.getLanguageManager().getComponent("gui.takeQuestChoose.title", null )).open(clickHandler.viewer(), arguments);
                                }
                        ));
    ...
    public ChestPane getTakeQuestPaneOfCategory(ChestPane pane, InterfaceView<ChestPane, PlayerViewer> view, Category category) {
            ...
            ArrayList<ItemStackElement<ChestPane>> list = new ArrayList<>() {
                {
                    ...
                    for (final Quest quest : main.getQuestManager().getAllQuestsWithVisibilityEvaluations(questPlayer)) {
                        if (quest.isTakeEnabled() && quest.getCategory().getCategoryFullName().equalsIgnoreCase(category.getCategoryFullName())) {
                            ...
                            List<String> loreStringList = main.getLanguageManager().getStringList("gui.takeQuestChoose.button.questPreview.lore", player, quest);
                            List<Component> lore = new ArrayList<>();
                            ...
                            if (questPlayer != null && questPlayer.hasAcceptedQuest(quest)) {
                                itemMeta.displayName(main.getLanguageManager().getComponent("gui.takeQuestChoose.button.questPreview.name-if-accepted", player, quest));
                            }else{
                                itemMeta.displayName(main.getLanguageManager().getComponent("gui.takeQuestChoose.button.questPreview.name-if-not-accepted", player, quest));
                            }
            ...
            ArrayList<ItemStackElement<ChestPane>> list = new ArrayList<>() {
                {
                    ...
                    for (final ActiveQuest activeQuest : questPlayer.getActiveQuests()) {
                        ...
                        itemMeta.displayName(main.getLanguageManager().getComponent("gui.abortQuestChoose.button.abortQuestPreview.name", player, activeQuest));
                        itemMeta.lore(main.getLanguageManager().getComponentList("gui.abortQuestChoose.button.abortQuestPreview.lore", player, activeQuest));

                        materialToUse.setItemMeta(itemMeta);

                        add(ItemStackElement.of(materialToUse,
                                // Handle click
                                (clickHandler) -> {
