    ...
    public static void handleCommands(NotQuests main, PaperCommandManager<CommandSender> manager, Command.Builder<CommandSender> addTriggerBuilder) {
        manager.command(addTriggerBuilder.literal("BEGIN")
                ...
                .handler((context) -> {
                    ...
                    BeginTrigger beginTrigger = new BeginTrigger(main, quest, quest.getTriggers().size() + 1, action, applyOn, worldString, 1);

                    quest.addTrigger(beginTrigger);

                    audience.sendMessage(MiniMessage.miniMessage().parse(
                            NotQuestColors.successGradient + "BEGIN Trigger successfully added to Quest " + NotQuestColors.highlightGradient
