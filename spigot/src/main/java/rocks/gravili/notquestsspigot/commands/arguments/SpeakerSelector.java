...
package rocks.gravili.notquestsspigot.commands.arguments;
...
import rocks.gravili.notquestsspigot.conversation.Speaker;
...
public class SpeakerSelector<C> extends CommandArgument<C, Speaker> { //0 = Quest
    ...
    protected SpeakerSelector(
        ...
        super(required, name, new SpeakerParser<>(main, conversationContext), defaultValue, Speaker.class, suggestionsProvider);
    }


    public static @NonNull <C> Builder<C> newBuilder(final @NonNull String name, final NotQuests main, final String conversationContext) {
        return new Builder<>(name, main, conversationContext);
    }

    public static <C> @NonNull CommandArgument<C, Speaker> of(final @NonNull String name, final NotQuests main, final String conversationContext) {
        return SpeakerSelector.<C>newBuilder(name, main, conversationContext).asRequired().build();
    }

    public static <C> @NonNull CommandArgument<C, Speaker> optional(final @NonNull String name, final NotQuests main, final String conversationContext) {
    ...
    public static <C> @NonNull CommandArgument<C, Speaker> optional(
        ...
        return SpeakerSelector.<C>newBuilder(name, main, conversationContext).asOptionalWithDefault("" + speaker.getSpeakerName()).build();
    }
    ...
    public static final class SpeakerParser<C> implements ArgumentParser<C, Speaker> {
        ...
        @Override
        public @NonNull ArgumentParseResult<Speaker> parse(@NonNull CommandContext<@NonNull C> context, @NonNull Queue<@NonNull String> inputQueue) {
            if (inputQueue.isEmpty()) {
                return ArgumentParseResult.failure(new NoInputProvidedException(SpeakerParser.class, context));
            }
            ...
            return ArgumentParseResult.failure(new IllegalArgumentException("Speaker '" + input + "' was not found in conversation " + conversation.getIdentifier() + "!"));
